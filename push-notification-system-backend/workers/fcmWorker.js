...
deviceData&&id?
  ...
  lockRenewTime: 60000});
worker.on('active', async ({ jobId }) => await updateStatusField(job.data.messagePayload.id, 'sending'));
  worker.on('failed',async (job, err) =>{await updateStatusField(job.data.messagePayload.id, 'failed'); console.error(`Job ${job.id} failed:`, err)});
